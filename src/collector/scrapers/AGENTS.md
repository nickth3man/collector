...
- Use `self.update_progress(progress_pct, operation_name)` for user-visible state
- Progress values are percent integers, operation text should be short, action-first
- Persist metadata with `save_metadata(...)`, persist file records with `save_file_record(...)`
- File entries appended to `result["files"]` use: `file_path`, `file_type`, `file_size`
...
- Delay source is `min_delay` / `max_delay`, wired from `SCRAPER_IG_DELAY_MIN` and `SCRAPER_IG_DELAY_MAX`
...
- Reuse base helpers for metadata/file DB writes, avoid duplicate persistence logic
- Instantiate new scraper where jobs are dispatched, pass `db_path`, `download_dir`, `progress_callback`
